...
class OnlineWindowBank:
    ...
    def __init__(
        ...
        self.raw_ring = base_module.empty_float((len(input_contexts), self.window_size))
        self.raw_write_idx = 0
        self.raw_filled_frames = 0
        self.raw_frames_since_emit = 0
        self.base_window_count = 0

        self.tmp_window = base_module.empty_float((self.window_size,))
        self.tmp_feature_map = {}
        for input_ctx in input_contexts:
            self.tmp_feature_map[input_ctx["name"]] = base_module.empty_float((self.window_size,))

        if self.zero_guard_enabled:
            self.zero_seq_ring = base_module.empty_float((len(input_contexts), self.max_seq_length, self.window_size))
            self.tmp_zero_seq = base_module.empty_float((self.max_seq_length, self.window_size))
        else:
            self.zero_seq_ring = None
            self.tmp_zero_seq = None
    ...
    def _emit_base_window(self, status_ctx):
        ...
        freq_total = 0.0
        for input_idx, input_ctx in enumerate(self.input_contexts):
            self.base.expand_ring_window(self.raw_ring[input_idx], self.raw_write_idx, self.tmp_window)
            source_index = int(input_ctx["source_index"])
            status_ctx.update_raw_anomaly(source_index, self.tmp_window)
            freq_total += self.base.mean_1d(self.tmp_window)
            if self.zero_guard_enabled:
                self.zero_seq_ring[input_idx][self.zero_seq_write_idx] = self.tmp_window
            self.base.apply_feature_mode_1d(
                self.tmp_window,
                self.feature_mode,
    ...
    def expand_zero_guard_sequence(self, input_idx):
        """中文注释：展开指定输入路的 zero guard 原始序列。"""
        self.base.expand_sequence_ring(
            self.zero_seq_ring[input_idx],
            self.zero_seq_write_idx,
            self.tmp_zero_seq,
        )
        return self.tmp_zero_seq
