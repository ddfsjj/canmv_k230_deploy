...
class UartDrynessSender:
    def __init__(self, uart_cfg):
        ...
        self.header = int(uart_cfg.get("header", 0xAA)) & 0xFF
        self.tail = int(uart_cfg.get("tail", 0xFF)) & 0xFF
        ...
        try:
            ...
            self.enabled = True
            print(
                "UART sender enabled: UART{}, {} bps, tx_pin={}, rx_pin={}".format(
    ...
    def _encode_int32_frame(self, values):
        payload = bytearray()
        ...
        for i in range(count):
            ...
            if i < len(values):
                ...
                if is_finite_number(raw):
                    v = clamp_int32(int(round(float(raw))))
            payload.extend(struct.pack("<i", int(v)))
    ...
    def send_scaled_prediction(self, pred_value):
        ...
        if is_finite_number(pred_value):
            scaled = float(pred_value) * self.scale
        values = [scaled]
        frame = self._encode_int32_frame(values)
        try:
...
def build_dataset(cfg, root, max_samples=None):
    ...
    for csv_file in list_csv_files(data_dir):
        ...
        for start in range(0, signal.size - base_window + 1, base_step):
            ...
            # Stream sequence emission so max_samples can stop early.
