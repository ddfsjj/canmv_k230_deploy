...
def normalize_feature_mode(feature_mode):
    text = str(feature_mode or "raw").strip().lower().replace("-", "_").replace(" ", "_")
    if text in {"window_demean", "demean", "window_mean_center", "remove_mean"}:
...
def normalize_model_type(model_type):
    text = str(model_type or "cnn").strip().lower()
    text = text.replace("-", "_")
    aliases = {
