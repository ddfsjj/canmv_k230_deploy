...
class RuntimeStatusContext:
    ...
    def __init__(self, cfg, runtime_cfg, channel_count, binding_count):
        ...
        self.raw_anomaly = guards.RawChannelAnomalyState(
            guards.get_raw_anomaly_config(cfg),
            channel_count,
        )
        self.channel_raw_error_codes = [protocol.RAW_ANOMALY_OK] * int(channel_count)
        self.postprocessor = guards.create_runtime_postprocessor(
            cfg,
...
def build_raw_anomaly_state(cfg, channel_count):
    """中文注释：创建原始通道异常状态机。"""
    return guards.RawChannelAnomalyState(cfg, channel_count)


def build_postprocessor(cfg, channel_count):
    """中文注释：创建统一后处理器。"""
    return guards.create_runtime_postprocessor(cfg, channel_count=channel_count)
