...
def build_uart_value_parser(uart_sender, channel_count, online_cfg):
...
class UartOnlineInputReader:
    ...
    def __init__(self, uart_sender, channel_count, online_cfg, online_print, quiet=False):
        self.uart_sender = uart_sender
        self.channel_count = int(channel_count)
        self.online_print = online_print
        self.quiet = bool(quiet)
        self.parser = build_uart_value_parser(uart_sender, channel_count, online_cfg)
    ...
    def _log_uart_read(self, rx_now_us, raw):
        ...
        if self.last_uart_read_us is not None:
            read_interval_ms = platform.diff_us(rx_now_us, self.last_uart_read_us) / 1000.0
    ...
    def _log_outer_frames(self, rx_now_us, raw, frames, total_rx_frames):
        ...
        if self.last_outer_rx_us is not None:
            outer_interval_ms = platform.diff_us(rx_now_us, self.last_outer_rx_us) / 1000.0
    ...
    def _log_small_frames(self, rx_now_us, raw, frames, total_rx_frames):
        ...
        if self.last_small_rx_us is not None:
            small_interval_ms = platform.diff_us(rx_now_us, self.last_small_rx_us) / 1000.0
        if self.debug_outer_rx:
