...
class OnlineWindowBank:
    ...
    def __init__(
        ...
        if self.zero_guard_enabled:
            self.zero_seq_ring = numeric.empty_float((len(input_contexts), self.max_seq_length, self.window_size))
            self.tmp_zero_seq = numeric.empty_float((self.max_seq_length, self.window_size))
        else:
    ...
    def _emit_base_window(self, status_ctx):
        ...
        for input_idx, input_ctx in enumerate(self.input_contexts):
            ...
            freq_total += numeric.mean_1d(self.tmp_window)
            if self.zero_guard_enabled:
