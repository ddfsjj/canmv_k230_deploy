...
# 大将军平时直接运行这个文件时，默认读取新版统一 runtime 配置。
# 旧 k230_config_* 只作为兼容参考，放在 configs/legacy/ 下。
...
def detect_root():
    ...
    for c in ordered:
        if (
            ...
            return c
        if exists(join_path(c, "configs/runtime.json")):
            return c
        if exists(join_path(c, "configs/legacy/auto_start_config.json")):
            return c
        if exists(join_path(c, "configs/legacy/k230_config_cnn_tcn.json")):
            return c
